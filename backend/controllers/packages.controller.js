...
export const createPackage = async (req, res) => {
  ...
  if (!title || !description || !destination || !price || !duration_days || !max_participants || !category) {
  ...
  try {
    const { rows } = await q(
      ...
      [title, description, destination, price, duration_days,
       max_participants, image_url || null, category, start_date || null,
       end_date || null, organizer_id || null, is_featured, is_active]
    );
